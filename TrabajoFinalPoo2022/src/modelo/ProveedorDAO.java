...
public class ProveedorDAO {
	...
	public ArrayList<Proveedor> getAll() {
		...
		try {
			while (rs.next()) {
				...
				proveedores
						.add(new Proveedor(cuitPersona, nombrePersona, telefonoPersona, direccionPersona, razonSocial));
			}
	...
	public Integer insert(Proveedor p) {
		return BaseDatos.getInstance().alta("proveedores", "cuit,nombre,telefono,direccion,razon_social",
				"'" + p.getCuit() + "','" + p.getNombre() + "','" + p.getTelefono() + "','" + p.getDireccion() + "','"
						+ p.getRazon_social() + "'");
	}

	public Boolean update(Proveedor p) {
		return BaseDatos.getInstance().update("proveedores", "cuit", p.getCuit(),
				"nombre = '" + p.getNombre() + "', telefono = '" + p.getTelefono() + "',direccion = '"
						+ p.getDireccion() + "',razon_social='" + p.getRazon_social() + "'");
	}
